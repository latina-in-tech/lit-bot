    ...
    if(user:=await retrieve_user_by_telegram_id(user_telegram_id)):
        ...
        if user_role_id:
            ...
            if await update_user(user):
                message = f'L\'utente @{username} ({user_telegram_id}) ha ora il ruolo di <b>{user_role_name}</b> \U00002705'
                await update.message.reply_text(text=message, parse_mode=ParseMode.HTML)
            else:
                await update.message.reply_text(text='\U0000274C Errore durante l\'aggiornamento del ruolo dell\'utente.')
        else:
            await update.message.reply_text(text='\U000026A0 Ruolo dell\'utente non corretto!')
            return
    else:
